...
import { X, Mail, Lock, Eye, EyeOff, User, Phone, Sparkles, ArrowLeft, Briefcase, ShoppingBag, MapPin, Camera, Upload } from "lucide-react"
...
const AuthFlow: React.FC<AuthFlowProps> = ({ isOpen, onClose, onAuthComplete }) => {
  ...
  const serviceCategories = [
    'Cleaning & Housekeeping', 'AC Repair & Maintenance', 'Electrical Services',
    'Plumbing Services', 'Carpentry & Furniture', 'Painting & Wall Care',
    'Appliance Installation', 'Computer & Electronics Repair', 'Auto Services',
    'Beauty & Personal Care', 'Pest Control', 'Moving & Packing',
    'Gardening & Landscaping', 'CCTV & Security', 'Water Purifier & RO',
    'Sofa & Carpet Cleaning', 'Other Services'
  ]
  ...
  const handleImageUpload = useCallback((
    ...
    forUserType: 'customer' | 'provider'
  ) => {
    const files = e.target.files
    if (!files || files.length === 0) return

    if (type === 'profile') {
      if (forUserType === 'customer') {
      ...
      setProviderData(prev => ({
        ...prev,
        portfolioImages: [...prev.portfolioImages, ...Array.from(files)]
      }))
    }
  }, [])
  ...
  const isCredentialsValid = useCallback(() => {
    ...
    if (authMode === 'signin') return baseValidation
    return (
      credentialsData.name && baseValidation &&
  ...
  const isCustomerProfileValid = useCallback(() => {
    return Boolean(customerProfileData.location)
  }, [customerProfileData])

  const isProviderDetailsValid = useCallback(() => {
    return Boolean(
      providerData.businessName && providerData.serviceCategory &&
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      if (isCompletingGoogleProviderProfile) {
        ...
        const identifier = (
          loginMethod === 'email' ? credentialsData.email : credentialsData.phone
        ).trim()
        await login(identifier, credentialsData.password, userType)
      } else if (userType === 'customer') {
        await registerCustomer({
          userName: credentialsData.email
            ? credentialsData.email.split('@')[0]
            : credentialsData.name.replace(/\s+/g, '').toLowerCase(),
          email: credentialsData.email,
          password: credentialsData.password,
          fullName: credentialsData.name,
          phone: credentialsData.phone || '9999999999',
          profileImage: customerProfileData.profileImage,   // ✅ always fresh via ref
        })
      } else {
    ...
    isCompletingGoogleProviderProfile,
    authMode,
    credentialsData,
    userType,
    loginMethod,
    customerProfileData,   // ✅ included — re-creates when image changes
    providerData,           // ✅ included — re-creates when image changes
  ...
  // ✅ Keep a ref that always mirrors the latest handleSubmit.
  // This is the standard React pattern for calling a fresh callback
  // from inside a stale closure without re-creating the outer callback.
  ...
  const handleNext = useCallback(() => {
    ...
    switch (currentStep) {
      ...
      case 'credentials':
        if (authMode === 'signup') {
          setCurrentStep(userType === 'customer' ? 'profileSetup' : 'providerDetails')
        } else {
          handleSubmitRef.current()   // ✅ always the latest handleSubmit
        }
        break
      case 'profileSetup':
        handleSubmitRef.current()     // ✅ always the latest handleSubmit
  ...
  }, [currentStep, userType, authMode]) // deps are minimal and correct

  const handleBack = useCallback(() => {
    setErrorMessage(null)
    switch (currentStep) {
      case 'userType':      setCurrentStep('welcome');      break
      case 'credentials':   setCurrentStep('userType');     break
      case 'profileSetup':  setCurrentStep('credentials');  break
      case 'providerDetails': setCurrentStep('credentials'); break
    }
  }, [currentStep])
  ...
  const handleSocialLogin = useCallback(async (provider: 'google') => {
    ...
    const launchGoogle = () => {
      ...
      if (!googleObj?.accounts?.id) {
        setErrorMessage('Google SDK failed to initialize.')
        return
      }
      googleObj.accounts.id.initialize({
        ...
        callback: async (response: { credential?: string }) => {
          ...
          try {
            ...
            onAuthComplete({ name: 'Google User', isProvider: userType === 'provider' })
            onClose()
          } catch {
    ...
    if (!(window as any).google?.accounts?.id) {
      ...
      script.onerror = () => setErrorMessage('Failed to load Google Sign-In SDK.')
  ...
  const canProceed = () => {
    switch (currentStep) {
      ...
      case 'credentials':   return Boolean(isCredentialsValid())
      case 'profileSetup':  return Boolean(isCustomerProfileValid())
      case 'providerDetails': return Boolean(isProviderDetailsValid())
  ...
  const renderWelcomeStep = () => (
    <div className="text-center space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-lg">Your trusted local services marketplace</p>
      </div>
      <div className="space-y-4">
        <Button
          onClick={() => { setAuthMode('signin'); handleNext() }}
        ...
        <Button
          onClick={() => { setAuthMode('signup'); handleNext() }}
          variant="outline"
  ...
  const renderUserTypeStep = () => (
    <div className="space-y-8">
      <div className="text-center">
        <h2 className="text-2xl font-bold mb-2">{authMode === 'signin' ? 'Sign in as' : 'Join as'}</h2>
        <p className="text-muted-foreground">
          {authMode === 'signin' ? 'Select your account type' : 'Choose how you want to use ServeEase'}
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {(['customer', 'provider'] as UserType[]).map((type) => (
          <button
            key={type}
          ...
          >
            <div className="flex flex-col items-center space-y-4">
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${
                userType === type ? 'bg-gradient-to-r from-primary to-sage-700' : 'bg-muted'
              }`}>
                {type === 'customer'
                  ? <ShoppingBag size={24} className={userType === type ? 'text-primary-foreground' : 'text-muted-foreground'} />
                  : <Briefcase   size={24} className={userType === type ? 'text-primary-foreground' : 'text-muted-foreground'} />
                }
              </div>
              <div className="text-center">
                <h3 className="text-lg font-semibold">
                  {type === 'customer' ? "I'm a Customer" : "I'm a Service Provider"}
                </h3>
  ...
  const renderCredentialsStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold mb-2">{authMode === 'signin' ? 'Sign In' : 'Create Account'}</h2>
        <p className="text-muted-foreground">
          {authMode === 'signin'
      ...
      <div className="relative">
        <div className="absolute inset-0 flex items-center">
          <div className="w-full border-t border-white/10 dark:border-white/20" />
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 p-1 bg-muted/20 rounded-xl">
        {(['email', 'phone'] as LoginMethod[]).map((method) => (
          <Button
            key={method}
            type="button"
            variant={loginMethod === method ? 'default' : 'ghost'}
            onClick={() => setLoginMethod(method)}
            className={`rounded-xl ${loginMethod === method ? 'bg-primary text-primary-foreground' : ''}`}
          >
            {method === 'email' ? <Mail size={16} className="mr-2" /> : <Phone size={16} className="mr-2" />}
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleNext() }} className="space-y-4">
        {authMode === 'signup' && (
          <div>
            ...
            <div className="relative mt-1">
              ...
              <Input id="name" value={credentialsData.name}
                onChange={(e) => handleCredentialsChange('name', e.target.value)}
                placeholder="Enter your full name" className="h-12 pl-12 rounded-xl modal-internal-glass" required />
            </div>
          </div>
        )}
        <div>
          ...
          <div className="relative mt-1">
            ...
            <Input id={loginMethod} type={loginMethod === 'email' ? 'email' : 'tel'}
              ...
              className="h-12 pl-12 rounded-xl modal-internal-glass" required />
          </div>
        </div>
        <div>
          ...
          <div className="relative mt-1">
            ...
            <Input id="password" type={showPassword ? 'text' : 'password'} value={credentialsData.password}
              onChange={(e) => handleCredentialsChange('password', e.target.value)}
              placeholder="Enter your password" className="h-12 pl-12 pr-12 rounded-xl modal-internal-glass" required />
            <Button type="button" variant="ghost" size="icon"
              onClick={() => setShowPassword(!showPassword)}
              className="absolute right-2 top-1/2 -translate-y-1/2 hover:bg-white/10 rounded-xl w-8 h-8">
        ...
        {authMode === 'signup' && (
          <div>
            ...
            <div className="relative mt-1">
              <Lock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <Input id="confirmPassword" type="password" value={credentialsData.confirmPassword}
                onChange={(e) => handleCredentialsChange('confirmPassword', e.target.value)}
                placeholder="Confirm your password" className="h-12 pl-12 rounded-xl modal-internal-glass" required />
            </div>
  ...
  const renderCustomerProfileSetup = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">Let's personalize your experience on ServeEase</p>
      </div>
      <div className="text-center">
        <div className="relative inline-block">
          <div className="w-24 h-24 rounded-3xl bg-gradient-to-r from-primary to-sage-700 flex items-center justify-center mx-auto overflow-hidden">
            {customerProfileData.profileImage ? (
              <img src={URL.createObjectURL(customerProfileData.profileImage)} alt="Profile"
                className="w-full h-full object-cover" />
            ) : (
              <Camera size={32} className="text-primary-foreground" />
            )}
          </div>
          <label htmlFor="customerProfileImage"
            className="absolute -bottom-2 -right-2 w-8 h-8 bg-primary rounded-full flex items-center justify-center cursor-pointer hover:bg-sage-700 transition-colors">
            <Upload size={16} className="text-primary-foreground" />
          </label>
          <input id="customerProfileImage" type="file" accept="image/jpeg,image/jpg,image/png"
            onChange={(e) => handleImageUpload(e, 'profile', 'customer')} className="hidden" />
        </div>
        <p className="text-sm text-muted-foreground mt-3">
          {customerProfileData.profileImage
            ? `Selected: ${customerProfileData.profileImage.name}`
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleNext() }} className="space-y-6">
        <div>
          ...
          <div className="relative mt-1">
            ...
            <Input id="location" value={customerProfileData.location}
              onChange={(e) => handleCustomerProfileChange('location', e.target.value)}
              placeholder="City, State" className="h-12 pl-12 rounded-xl modal-internal-glass" required />
          </div>
        </div>
        <div>
          <Label className="text-base font-medium mb-3 block">Services You're Interested In (Optional)</Label>
          ...
          <p className="text-xs text-muted-foreground mt-2">We'll show you relevant services and offers</p>
        </div>
      </form>
    </div>
  )
  ...
  const renderProviderDetailsStep = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">Tell us about your business and services</p>
      </div>
      <div className="text-center">
        <div className="relative inline-block">
          <div className="w-24 h-24 rounded-3xl bg-gradient-to-r from-primary to-sage-700 flex items-center justify-center mx-auto overflow-hidden">
            {providerData.profileImage ? (
              <img src={URL.createObjectURL(providerData.profileImage)} alt="Profile"
                className="w-full h-full object-cover" />
            ) : (
              <Camera size={32} className="text-primary-foreground" />
            )}
          </div>
          <label htmlFor="providerProfileImage"
            className="absolute -bottom-2 -right-2 w-8 h-8 bg-primary rounded-full flex items-center justify-center cursor-pointer hover:bg-sage-700 transition-colors">
            <Upload size={16} className="text-primary-foreground" />
          </label>
          <input id="providerProfileImage" type="file" accept="image/jpeg,image/jpg,image/png"
            onChange={(e) => handleImageUpload(e, 'profile', 'provider')} className="hidden" />
        </div>
        <p className="text-sm text-muted-foreground mt-3">
          {providerData.profileImage
            ? `Selected: ${providerData.profileImage.name}`
            : 'Upload your profile picture (optional)'}
        </p>
      </div>
      <form onSubmit={(e) => { e.preventDefault(); handleNext() }} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input id="businessName" value={providerData.businessName}
              onChange={(e) => handleProviderDataChange('businessName', e.target.value)}
              placeholder="Your business name" className="h-12 rounded-xl modal-internal-glass mt-1" required />
          </div>
          <div>
            <Label htmlFor="serviceCategory">Service Category *</Label>
            <Select value={providerData.serviceCategory}
              onValueChange={(v) => handleProviderDataChange('serviceCategory', v)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input id="experience" value={providerData.experience}
              onChange={(e) => handleProviderDataChange('experience', e.target.value)}
              placeholder="e.g., 5 years" className="h-12 rounded-xl modal-internal-glass mt-1" required />
          </div>
          <div>
            <Label htmlFor="providerLocation">Location *</Label>
            <Input id="providerLocation" value={providerData.location}
              onChange={(e) => handleProviderDataChange('location', e.target.value)}
              placeholder="Your city" className="h-12 rounded-xl modal-internal-glass mt-1" required />
          </div>
        </div>
        <div>
          <Label htmlFor="serviceArea">Service Area</Label>
          <Input id="serviceArea" value={providerData.serviceArea}
            onChange={(e) => handleProviderDataChange('serviceArea', e.target.value)}
            placeholder="Areas you serve (e.g., 10km radius)" className="h-12 rounded-xl modal-internal-glass mt-1" />
        </div>
        <div>
          <Label htmlFor="description">Service Description *</Label>
          <Textarea id="description" value={providerData.description}
            ...
            className="min-h-[100px] rounded-xl modal-internal-glass mt-1 resize-none" required />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <Label htmlFor="pricing">Starting Price Range</Label>
            <Input id="pricing" value={providerData.pricing}
              onChange={(e) => handleProviderDataChange('pricing', e.target.value)}
              placeholder="e.g., ₹500-2000" className="h-12 rounded-xl modal-internal-glass mt-1" />
          </div>
          <div>
            <Label htmlFor="availability">Availability</Label>
            <Input id="availability" value={providerData.availability}
              onChange={(e) => handleProviderDataChange('availability', e.target.value)}
              placeholder="e.g., Mon-Fri 9AM-6PM" className="h-12 rounded-xl modal-internal-glass mt-1" />
          </div>
        </div>
        <div>
          <Label htmlFor="certifications">Certifications & Licenses</Label>
          <Input id="certifications" value={providerData.certifications}
        ...
        <div className="flex items-center space-x-2">
          <Checkbox id="acceptTerms" checked={providerData.acceptTerms}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="absolute inset-0 modal-backdrop" onClick={onClose} />
          <motion.div
          ...
          >
            <div className="modal-glass rounded-3xl p-8 shadow-2xl">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex space-x-2">
                    {['welcome', 'userType', 'credentials',
                      ...(authMode === 'signup'
                        ? userType === 'customer' ? ['profileSetup'] : ['providerDetails']
                        : [])
                    ].map((step, index) => (
                      <div key={step} className={`w-2 h-2 rounded-full transition-colors ${
                        ['welcome','userType','credentials','profileSetup','providerDetails']
                          .indexOf(currentStep) >= index ? 'bg-primary' : 'bg-muted'
                      }`} />
                    ))}
                  </div>
                </div>
              ...
              {/* Error message */}
              {errorMessage && (
                <div className="mt-4 p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm text-center">
                  {errorMessage}
                </div>
              )}
                  ...
                  <Button onClick={handleNext} disabled={!canProceed() || isLoading}
                    ...
                    {isLoading ? (
                      <div className="flex items-center space-x-2">
                        ...
                        <span>
                          {(currentStep === 'profileSetup' || currentStep === 'providerDetails' ||
                            (currentStep === 'credentials' && authMode === 'signin'))
