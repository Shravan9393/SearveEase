...
// import { motion, AnimatePresence } from "motion/react"
// import { 
//   TrendingUp, Eye, CheckCircle2, DollarSign, Star, Clock,
//   MapPin, Users, ArrowUp, ArrowDown, Activity, BarChart3,
...
//   // The previous deps [user.location?.city, user.location?.state] were unused
//   // by the API call itself and caused unnecessary re-fetches.
//   useEffect(() => {
//     const loadDashboard = async () => {
//       try {
//         setIsLoadingDashboard(true)
...
//     loadDashboard()
//   }, []) // FIX: was [user.location?.city, user.location?.state]
...
//   const handleDenyNotification = (id: string) => {
//     setNotifications(prev => prev.map(n => n.id === id ? { ...n, status: 'denied' as const } : n))
//   }
...
//                     ...service,
//                     ...updatedService,
//                     // Preserve live stats from the existing entry
//                     bookings: service.bookings,
//                     revenue: service.revenue,
//                     views: service.views,
//                     rating: service.rating,
...
//                   alt={user.name}
//                   className="w-16 h-16 rounded-full object-cover border-2 border-primary/50"
//                   style={{ boxShadow: '0 0 15px rgba(88, 129, 87, 0.4)' }}
//                 />
//                 <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-white dark:border-gray-900" />
//               </div>
//               <div>
...
//                 </defs>
//                 <CartesianGrid strokeDasharray="3 3" stroke="rgba(163, 177, 138, 0.1)" />
//                 <XAxis dataKey="month" stroke="rgba(163, 177, 138, 0.5)" style={{ fontSize: '12px' }} />
//                 <YAxis stroke="rgba(163, 177, 138, 0.5)" style={{ fontSize: '12px' }} />
//                 <Tooltip contentStyle={{ backgroundColor: 'rgba(218, 215, 205, 0.9)', border: '1px solid rgba(163, 177, 138, 0.2)', borderRadius: '12px', backdropFilter: 'blur(10px)' }} />
...
//               <BarChart data={activityData}>
//                 <CartesianGrid strokeDasharray="3 3" stroke="rgba(163, 177, 138, 0.1)" />
//                 <XAxis dataKey="day" stroke="rgba(163, 177, 138, 0.5)" style={{ fontSize: '12px' }} />
//                 <YAxis stroke="rgba(163, 177, 138, 0.5)" style={{ fontSize: '12px' }} />
//                 <Tooltip contentStyle={{ backgroundColor: 'rgba(218, 215, 205, 0.9)', border: '1px solid rgba(163, 177, 138, 0.2)', borderRadius: '12px', backdropFilter: 'blur(10px)' }} />
//                 <Bar dataKey="queries" fill="#A3B18A" radius={[8, 8, 0, 0]} />
//                 <Bar dataKey="bookings" fill="#588157" radius={[8, 8, 0, 0]} />
...
//             onClick={() => setIsQueriesModalOpen(true)}
//             className="glass-card p-6 rounded-2xl border border-primary/30 hover:border-primary/50 transition-all text-left group"
//             style={{ boxShadow: '0 0 15px rgba(88, 129, 87, 0.2), inset 0 0 10px rgba(88, 129, 87, 0.05)' }}
//           >
//             <div className="flex items-center gap-4">
//               <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
//                 <Bell size={24} className="text-primary" />
...
//       {/* FIX: ManageServicesModal now receives onServiceAdded + onServiceUpdated callbacks
//               so the dashboard listings card updates immediately after changes */}
//       <AnimatePresence>
//         {isManageServicesModalOpen && (
//           <ManageServicesModal
//             isOpen={isManageServicesModalOpen}
...
//       {/* FIX: Wrapped in AnimatePresence so enter/exit animations work correctly */}
//       <AnimatePresence>
