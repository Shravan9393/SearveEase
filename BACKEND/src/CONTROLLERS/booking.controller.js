...
const BOOKING_STATUS = ["pending", "confirmed", "in-progress", "completed", "cancelled"];
...
const CreateBooking = asyncHandler(async (req, res) => {
  ...
  if (!address?.name || !address?.phone || !address?.addressLine1 || !address?.city || !address?.state || !address?.pincode) {
    throw new ApiError(StatusCodes.BAD_REQUEST, "Complete address details are required");
  }
  ...
  await Notification.create({
    ...
    metadata: {
      ...
      addressSummary: [address.addressLine1, address.city, address.state, address.pincode]
        .filter(Boolean)
        .join(", "),
  ...
  return new ApiResponse(res).success({ booking: populatedBooking }, "Booking created successfully", StatusCodes.CREATED);
});
...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  if (!BOOKING_STATUS.includes(status)) throw new ApiError(StatusCodes.BAD_REQUEST, "Invalid status");
  ...
  if (user.role !== "provider") {
    throw new ApiError(StatusCodes.FORBIDDEN, "Only providers can update booking status");
  }

  const providerProfile = await ProviderProfile.findOne({ userId: req.user._id });
  if (!providerProfile || providerProfile._id.toString() !== booking.providerProfileId.toString()) {
    throw new ApiError(StatusCodes.FORBIDDEN, "You can only manage your own booking requests");
  }

  if (!PROVIDER_ALLOWED_STATUSES.includes(status)) {
    throw new ApiError(StatusCodes.BAD_REQUEST, "Providers can only accept or decline booking requests");
  }
...
const cancelBooking = asyncHandler(async (req, res) => {
  ...
  if (!customerProfile || booking.customerProfileId.toString() !== customerProfile._id.toString()) {
    throw new ApiError(StatusCodes.FORBIDDEN, "You can only cancel your own bookings");
  }
